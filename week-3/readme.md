...
| _print team members to the console_ | button with unique id | event listener and callback to execute print_team_members | print_team_members: takes an array, loops over the array printing each entry (using template literals) | an array of team member objects -> ```{name: "string", age: number, profession: "string"}``` |
...
| _add a team member to array_ | 3 buttons & input fields with unique id's (name, age, profession) | an event listener & cb to read the new name from the dom & push it into the team array | add_team_member: takes in a number & 2 strings, returns a new team member object | none |
